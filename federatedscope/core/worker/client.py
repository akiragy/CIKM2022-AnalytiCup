...
class Client(Worker):
    ...
    def callback_funcs_for_evaluate(self, message: Message):
        ...
        if self.early_stopper.early_stopped and self._cfg.federate.method in [
            ...
            # Save Mid prediction result
            if self._cfg.data.type == 'cikmcup':
                # Evaluate
                training_round = message.state
                self.trainer.evaluate(target_data_split_name='test')
                self.trainer.save_prediction_during_eval(self._cfg.outdir, self.ID, self._cfg.model.task, training_round)
                logger.info(
                    f"Client #{self.ID} finished saving mid prediction results in {os.path.abspath(os.path.join(self._cfg.outdir, 'prediction' + str(training_round) + '.csv'))}")
        ...
        self.comm_manager.send(
            Message(msg_type='metrics',
                    ...
                    content=metrics))
        # # Save eval result
        # evalIdx = message.state
        # if self._cfg.data.type == 'cikmcup':
        #     # Evaluate
        #     self.trainer.evaluate(target_data_split_name='test')
        #     self.trainer.save_prediction_eval(self._cfg.outdir, self.ID, self._cfg.model.task, evalIdx)
        #     logger.info(f"EVAL-Client #{self.ID} finished saving prediction results in {os.path.abspath(os.path.join(self._cfg.outdir, 'prediction'+str(evalIdx)+'.csv'))}")
        #
